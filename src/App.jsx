...
const DB = {
  ...
  async insert(table, item) {
    try {
      ...
      console.error(`❌ DB.insert FAILED - Table: ${table}`, e.message, e); 
      throw e; // Throw error so caller can handle it
    }
  },
          ...
          <div className="modal">
            {modal==="addTeam"&&<>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                ...
                <div style={{display:"flex",gap:10,marginTop:8}}><button className="btn-primary" style={{flex:1}} onClick={async()=>{if(!form.name)return notify("Name required","error");try{const newId=uid();const teamData={id:newId,wins:0,losses:0,players:parseInt(form.players)||11,...form};await DB.insert(DB.KEYS.TEAMS,teamData);setTeams(ts=>[...ts,teamData]);closeModal();notify("Team added!");}catch(e){notify(`Add failed: ${e.message}`,"error");};}}>Add</button><button className="btn-ghost" onClick={closeModal}>Cancel</button></div>
              </div>
            </>}
            {modal==="addPlayer"&&<>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                ...
                <div style={{display:"flex",gap:10,marginTop:8}}><button className="btn-primary" style={{flex:1}} onClick={async()=>{if(!form.name)return notify("Required","error");const d={...form,matches:+form.matches||0,runs:+form.runs||0,avg:+form.avg||0,sr:+form.sr||0,wickets:+form.wickets||0,economy:+form.economy||0,catches:+form.catches||0,fifties:+form.fifties||0,hundreds:+form.hundreds||0,hs:+form.hs||0,ducks:+form.ducks||0,balls_faced:+form.balls_faced||0,fours:+form.fours||0,sixes:+form.sixes||0};if(editItem){try{await DB.update(DB.KEYS.PLAYERS,editItem.id,d);setPlayers(ps=>ps.map(p=>p.id===editItem.id?{...p,...d}:p));notify("Updated!");closeModal();}catch(e){notify(`Update failed: ${e.message}`,"error");}}else{try{const newId=uid();await DB.insert(DB.KEYS.PLAYERS,{id:newId,...d});setPlayers(ps=>[...ps,{id:newId,...d}]);notify("Added!");closeModal();}catch(e){notify(`Add failed: ${e.message}`,"error");}}}}>Save</button><button className="btn-ghost" onClick={closeModal}>Cancel</button></div>
              </div>
            </>}
            {modal==="addMatch"&&<>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                ...
                <div style={{display:"flex",gap:10,marginTop:8}}><button className="btn-primary" style={{flex:1}} onClick={async()=>{if(!form.team1||!form.team2)return notify("Select both teams","error");try{const newId=uid();const matchData={id:newId,status:"upcoming",innings:1,...form};await DB.insert(DB.KEYS.MATCHES,matchData);setMatches(ms=>[...ms,matchData]);closeModal();notify("Scheduled!");}catch(e){notify(`Schedule failed: ${e.message}`,"error");};}}>Schedule</button><button className="btn-ghost" onClick={closeModal}>Cancel</button></div>
              </div>
            </>}
            {modal==="addTournament"&&<>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                ...
                <div style={{display:"flex",gap:10,marginTop:8}}><button className="btn-primary" style={{flex:1}} onClick={async()=>{if(!form.name)return notify("Name required","error");try{const newId=uid();const tourData={id:newId,registered:[],createdBy:currentUser.id,...form,teams:parseInt(form.teams)||8};await DB.insert(DB.KEYS.TOURNAMENTS,tourData);setTournaments(ts=>[...ts,tourData]);closeModal();notify("Created!");}catch(e){notify(`Create failed: ${e.message}`,"error");};}}>Create</button><button className="btn-ghost" onClick={closeModal}>Cancel</button></div>
              </div>
            </>}
            {modal==="addGround"&&<>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                ...
                <div style={{display:"flex",gap:10,marginTop:8}}><button className="btn-primary" style={{flex:1}} onClick={async()=>{if(!form.name)return notify("Name required","error");try{const newId=uid();const groundData={id:newId,floodlights:form.floodlights!=="false",capacity:parseInt(form.capacity)||1000,facilities:[],...form};await DB.insert(DB.KEYS.GROUNDS,groundData);setGrounds(gs=>[...gs,groundData]);closeModal();notify("Added!");}catch(e){notify(`Add failed: ${e.message}`,"error");};}}>Add</button><button className="btn-ghost" onClick={closeModal}>Cancel</button></div>
              </div>
            </>}
            {modal==="addUser"&&<>
